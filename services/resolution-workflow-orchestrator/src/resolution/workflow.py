...
class ResolutionRunner:
    ...
    async def run(
        ...
        *,
        ...
        # ─── Saga compensation (stub for Stage 4a) ──────────────────────────
        saga_actions: list[dict[str, Any]] = []
        if (
            self.cfg.saga.enabled
            and failed_step_index is not None
            and self.cfg.saga.compensations
        ):
            saga_actions = await self._run_saga(failed_step_index, chain_outputs, process)
    ...
    async def _run_saga(
        ...
        """Stage 4b will fill this in. Stage 4a returns the planned actions
        without executing them — useful for audit even when no compensation
        is configured."""
        planned: list[dict[str, Any]] = []
        for comp in self.cfg.saga.compensations:
            if comp.on_failure_of_step == failed_step:
                planned.append({
                    "capability": comp.capability,
                    "skill": comp.skill,
                    "executed": False,
                    "reason": "Saga execution is Stage 4b — Stage 4a returns plan only",
                })
        return planned
