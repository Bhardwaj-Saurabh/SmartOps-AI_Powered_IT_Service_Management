"""FastAPI entrypoint for the SLA Monitor Agent."""
...
from sla_monitor.models import SLAInput
from sla_monitor.tools import ClockTimer, SLARulesEngine
from sla_monitor.workflow import SLARunner


_LOG = logging.getLogger("sla-monitor-agent")
_CFG_PATH = os.environ.get(
    "AGENT_CONFIG_PATH", "/app/services/sla-monitor-agent/configs/agent.yaml"
)
...
_clock = ClockTimer(_cfg.tools["clock_timer"].url, _cfg.tools["clock_timer"].timeout_seconds)
_rules = SLARulesEngine(_cfg.tools["sla_rules"].url, _cfg.tools["sla_rules"].timeout_seconds)

_runner = SLARunner(
    cfg=_cfg, gateway=_gateway, semantic=_semantic,
    clock=_clock, rules=_rules,
)


async def sla_handler(message: Message, task: Task) -> Task:
    ...
    for part in message.parts:
        if isinstance(part, DataPart):
            ...
            break
    try:
        inp = SLAInput.model_validate(payload)
    except Exception as exc:
        raise AgentError(f"Invalid calculate_sla_status payload: {exc}", step=1) from exc

    with audit_span("sla.run", audit_type=AuditType.PLATFORM):
        result = await _runner.run(inp)

    task.artifacts.append(
        TaskArtifact(name="sla_status", parts=[DataPart(data=result.model_dump(mode="json"))])
    )
...
_card = AgentCard(
    ...
    description="Calculate SLA compliance metrics and detect breaches",
    url=f"http://sla-monitor-agent:{_cfg.a2a.port}",
    version=_cfg.version,
    capabilities=AgentCapabilities(streaming=False, pushNotifications=False),
    skills=[AgentSkill(id=s.id, name=s.name, description=s.description, tags=["sla"]) for s in _cfg.a2a.skills],
    securitySchemes={
        "keycloak": {
            "type": "openIdConnect",
...
_handlers.register("calculate_sla_status", sla_handler)
...
_health = HealthCheck(
    service="sla-monitor-agent", version=_cfg.version,
    probes={
        ...
        "clock_timer": lambda: _probe_http(f"{_clock.base_url}/health"),
        "sla_rules":   lambda: _probe_http(f"{_rules.base_url}/health"),
    },
)
_auth = KeycloakAuth(
...
async def _lifespan(app: FastAPI):
    ...
    if _cfg.capability_registry.deregister_on_shutdown:
        try:
            await _semantic.deregister("calculate_sla_status")
